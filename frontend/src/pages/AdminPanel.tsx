...
const ClearanceInfo = ({ level, desc, color }: { level: string; desc: string; color: string }) => (
  <div className="space-y-1.5 group cursor-default">
    ...
    <p className="text-[10px] text-slate-500 font-medium leading-relaxed group-hover:text-slate-300 transition-colors uppercase tracking-tight">{desc}</p>
  </div>
);
...
export const AdminPanel: React.FC = () => {
  ...
  return (
    <div className="p-8">
      ...
      <div className="glass rounded-[2.5rem] p-10 border border-white/5 shadow-2xl min-h-[600px]">
        {activeTab === 'users' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 gap-4">
              {users.map((user) => (
                <div key={user.user_id} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center justify-between hover:bg-white/[0.07] transition-all group">
                  <div className="flex items-center gap-5">
                    <div className="w-12 h-12 bg-black/40 rounded-full flex items-center justify-center border border-white/10 text-slate-400 font-black text-lg">
                      {user.full_name?.charAt(0)}
                    </div>
                    <div>
                      <p className="font-black text-white text-lg tracking-tight uppercase">{user.full_name}</p>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right mr-4 hidden md:block">
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${user.status === 'approved' || user.status === 'active' ? 'text-emerald-500' : 'text-rose-500'}`}>
                        {user.status}
                      </span>
                    </div>
                    <button 
                    ...
                    <button className="p-3 bg-white/5 border border-white/10 rounded-xl text-slate-500 hover:text-rose-500 transition-all">
              ...
              {users.length === 0 && !loading && (
                <div className="py-20 text-center glass rounded-[2rem] border border-dashed border-white/5 opacity-30">
                  <Users className="w-16 h-16 mx-auto mb-4 text-slate-500" />
                  <p className="text-xl font-black uppercase tracking-[0.3em]">No Identities Found</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'audit' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
             <div>
                <h3 className="text-2xl font-black text-white uppercase tracking-tighter">System Audit Trail</h3>
                ...
                <tbody className="divide-y divide-white/5">
                  {auditLogs.map((log, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="p-6 text-xs font-mono text-slate-500">{new Date(log.timestamp).toLocaleString()}</td>
                      <td className="p-6">
                        <span className="text-xs font-bold text-slate-300">@{log.username}</span>
                      </td>
                      <td className="p-6">
                        <span className="text-[10px] font-black px-3 py-1 bg-white/5 rounded-full border border-white/10 text-white group-hover:border-accent-primary/30 transition-all uppercase tracking-widest">{log.action}</span>
                      </td>
                      <td className="p-6 text-xs text-slate-500 italic">{log.details}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'database' && (
          <div className="max-w-3xl space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="space-y-4">
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter">System Lifecycle</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white/5 border border-white/10 rounded-[2rem] p-8 space-y-6 group hover:border-accent-primary/20 transition-all">
                <div className="w-14 h-14 bg-accent-primary/10 rounded-2xl flex items-center justify-center text-accent-primary">
                  <Download className="w-7 h-7" />
                </div>
                <div>
                  <h4 className="text-lg font-black text-white uppercase tracking-tight">Full System Backup</h4>
                ...
                <button 
                  ...
                  className="w-full bg-white/5 hover:bg-accent-primary text-white py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all border border-white/10 group-hover:shadow-2xl group-hover:shadow-accent-primary/20"
                >
                  Initiate Backup
                </button>
              </div>

              <div className="bg-rose-500/5 border border-rose-500/10 rounded-[2rem] p-8 space-y-6 group hover:border-rose-500/20 transition-all">
                <div className="w-14 h-14 bg-rose-500/10 rounded-2xl flex items-center justify-center text-rose-500">
                  <AlertOctagon className="w-7 h-7" />
                </div>
                <div>
                  <h4 className="text-lg font-black text-white uppercase tracking-tight">Restore & Overwrite</h4>
                ...
                <button 
                  ...
                  className="w-full bg-rose-600/10 hover:bg-rose-600 text-rose-500 hover:text-white py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all border border-rose-500/20 group-hover:shadow-2xl group-hover:shadow-rose-600/20"
                >
                  Initiate Restore
                </button>
        ...
        <div className="max-w-7xl mx-auto h-full flex flex-col lg:flex-row gap-16 py-8 items-stretch">
          ...
          <div className="lg:w-1/3 space-y-10 animate-in fade-in slide-in-from-left-8 duration-1000">
            <div className="space-y-4">
              ...
              <h2 className="text-4xl font-black text-white tracking-tighter leading-tight uppercase">Security<br/>Provisioning</h2>
            ...
            <div className="glass border border-white/5 rounded-3xl p-8 space-y-8 bg-black/20">
          ...
          <div className="lg:w-2/3 animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
            <div className="glass border border-white/5 rounded-[3rem] p-12 shadow-2xl bg-gradient-to-br from-white/[0.02] to-transparent h-full relative overflow-hidden">
    ...
    <form onSubmit={handleSubmit} className="p-2 space-y-10">
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 px-1">
          ...
          <h4 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">Personal Identity</h4>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="group space-y-2">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-accent-primary/20 transition-all font-bold text-sm placeholder:text-slate-800"
          ...
          <div className="group space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/5 rounded-xl pl-10 pr-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-accent-primary/20 transition-all font-bold text-sm placeholder:text-slate-800"
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 px-1">
          ...
          <h4 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">Security & Clearance</h4>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="group space-y-2">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-bold text-sm placeholder:text-slate-800"
              placeholder="••••••••••••"
              required
            />
          </div>
