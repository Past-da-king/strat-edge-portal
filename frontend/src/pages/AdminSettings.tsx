    ...
    <div className="p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-400">Manage user access, project assignments, and system lifecycle.</p>
        </div>
      </div>
      ...
      <div className="glass rounded-3xl p-8 min-h-[500px]">
            ...
            {pendingUsers.length > 0 ? (
              <div className="grid grid-cols-1 gap-4">
                {pendingUsers.map(user => (
                  <div key={user.user_id} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-white text-lg">{user.full_name}</p>
                      <p className="text-sm text-slate-500 font-mono">@{user.username} • {user.role.toUpperCase()}</p>
                    </div>
                    <div className="flex gap-3">
                      <button 
                        onClick={() => handleApprove(user.user_id)}
                        className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-xl font-bold transition-all text-sm flex items-center gap-2"
                      >
                        APPROVE
                      </button>
                      <button className="bg-rose-600/20 hover:bg-rose-600/30 text-rose-500 px-6 py-2 rounded-xl font-bold border border-rose-500/20 transition-all text-sm">
                        REJECT
                      </button>
                    </div>
                  </div>
        ...
        {activeTab === 'access' && (
          <div className="space-y-8">
            <h3 className="text-xl font-bold text-white">Project Team Assignments</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Lead Project Manager</h4>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <p className="text-white font-bold">Assign Primary Lead</p>
                      <p className="text-xs text-slate-500 pr-4">This user will have full administrative rights over the project timeline.</p>
                    </div>
                  </div>
                  <CustomSelect 
                    value={newPmId}
                  ...
                  <button 
                    ...
                    className="w-full bg-accent-primary text-white py-3 rounded-xl font-bold text-xs hover:bg-accent-secondary transition-all shadow-lg shadow-accent-primary/20"
                  >
                    CHANGE LEAD PM
                  </button>
                </div>
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Project Team</h4>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 bg-accent-primary/10 rounded-full flex items-center justify-center text-accent-primary">
                      <Users className="w-6 h-6" />
                    ...
                    <div>
                      <p className="text-white font-bold">Manage Staff Access</p>
                      <p className="text-xs text-slate-500 pr-4">Grant specific users access to view or record data for this project.</p>
                    </div>
                  </div>
                  
                  <div className="max-h-40 overflow-y-auto mb-4 border border-white/5 rounded-xl p-3 space-y-1 bg-black/20 custom-scrollbar">
                    {approvedUsers.map(u => (
                      <label key={u.user_id} className="flex items-center gap-3 p-2.5 hover:bg-white/5 rounded-lg transition-all cursor-pointer group">
                        <input 
                          type="checkbox" 
                          ...
                          className="w-4 h-4 rounded border-white/10 bg-black/40 text-accent-primary focus:ring-accent-primary focus:ring-offset-0 transition-all cursor-pointer"
                        />
                        <span className="text-xs text-slate-400 group-hover:text-slate-200 font-bold transition-colors">{u.full_name}</span>
                      </label>
                    ))}
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-white/5 border border-white/10 text-white py-3 rounded-xl font-bold text-xs hover:bg-white/10 transition-all"
                  >
                    UPDATE PROJECT TEAM
                  </button>
                </div>
        ...
        {activeTab === 'tasks' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold text-white">Specific Task Assignments</h3>
                <p className="text-slate-500 text-sm">Assign project phases to specific personnel.</p>
              </div>
              <button 
                onClick={handleSaveTaskAssignments}
                className="bg-accent-primary hover:bg-accent-secondary text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-accent-primary/20"
              >
            ...
            <DenseTable headers={['Activity Name', 'Start Date', 'Responsible Person']}>
              {tasks.map((task: any) => (
                <DenseRow key={task.activity_id}>
                  <DenseCell flex={2}>
                    <span className="text-slate-200 font-bold uppercase tracking-tight text-sm">{task.activity_name}</span>
                  </DenseCell>
                  <DenseCell>
                    <span className="text-slate-500 font-mono text-[10px] bg-white/5 px-2 py-1 rounded border border-white/5">{task.planned_start}</span>
                  </DenseCell>
                  <DenseCell>
                    <CustomSelect 
...
const TabButton = ({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: any; label: string }) => (
  <button 
    onClick={onClick}
    className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all border whitespace-nowrap ${
      active 
